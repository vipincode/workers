...
export default function PermanentServiceReviewForm() {
  const [selectedRating, setSelectedRating] = useState(0);
  const { user } = useAuthStore();
  const navigate = useNavigate();

  // Select service_id from url
  const { serviceId: instantServiceId, bookedServiceId } = useParams();
  const serviceId = parseInt(atob(instantServiceId), 10);
  const bookedSrvId = parseInt(atob(bookedServiceId), 10);

  console.log(bookedSrvId);
